...
from models_eval.set_Custom import Model as Custom
...
class Model(Custom):
    ...
    def test(self):
        ...
        if self.epoch == self.opt["epochs"]:
            data = {"Time": [datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")], "Attr": [self.opt["attrs_pred"][0]], "Eval Mode": [self.opt["Custom_test_mode"]], "H(Y|A)": [self.ce], "Test Acc": [test_acc * 100]}
            utils.append_data_to_csv(data, os.path.join("result", self.opt["experiment"], self.opt["name"], "Custom_Baseline_trials.csv"))
    ...
    def load_weights(self, file_path):
        ...
        if torch.cuda.device_count() == 1 and "module" in list(ckpt["predictor"].keys())[0]:
            new_state_dict = {"predictor": {}}
            for key, value in ckpt["predictor"].items():
                new_key = key.replace("module.", "") 
                new_state_dict["predictor"][new_key] = value
            ckpt = new_state_dict
