...
class Generator(nn.Module):
    def __init__(self, enc_dim=64, enc_layers=5, enc_norm_fn="batchnorm", enc_acti_fn="lrelu", dec_dim=64, dec_layers=5, dec_norm_fn="batchnorm", dec_acti_fn="relu", n_attrs=1, shortcut_layers=1, inject_layers=1, img_size=224, dim_per_attr=5):
        ...
        for i in range(dec_layers):
            if i < dec_layers - 1:
                ...
                layers += [ConvTranspose2dBlock(n_in, 3, (4, 4), stride=2, padding=1, norm_fn="none", acti_fn="sigmoid")]
    ...
    def decode(self, zs, a):
        ...
        for i, layer in enumerate(self.dec_layers):
            ...
            if self.inject_layers > i:
                a_tile = a.view(a.size(0), -1, 1, 1).repeat(1, self.dim_per_attr, self.f_size * 2 ** (i + 1), self.f_size * 2 ** (i + 1))
