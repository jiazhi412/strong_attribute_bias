...
class ResNet50(nn.Module):
    def __init__(self, n_classes, weights=ResNet50_Weights.DEFAULT, hidden_size=2048, dropout=0.5):
        super().__init__()
        self.resnet = torchvision.models_eval.resnet50(weights=weights)
...
class ResNet50_base(nn.Module):
    ...
    def __init__(self, pretrained, hidden_size=2048, dropout=0.5):
        super().__init__()
        self.resnet = torchvision.models_eval.resnet50(pretrained=pretrained)
        self.resnet.fc = nn.Linear(2048, hidden_size)
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(dropout)
...
class Vgg16(nn.Module):
    def __init__(self, n_classes, weights=VGG16_Weights.DEFAULT, hidden_size=2048, dropout=0.5):
        super().__init__()
        self.vgg = torchvision.models_eval.vgg16(weights=weights)
