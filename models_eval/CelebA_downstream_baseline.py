...
from models_eval.set_CelebA import Model as CelebA
...
class Model(CelebA):
    ...
    def test(self):
        ...
        if self.epoch == self.opt["epochs"]:
            data = {"Time": [datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")], "Attr": [self.opt["attrs_pred"][0]], "Eval Mode": [self.opt["CelebA_test_mode"]], "H(Y|A)": [self.ce], "Test Acc": [test_acc * 100]}
            utils.append_data_to_csv(data, os.path.join("result", self.opt["experiment"], self.opt["name"], "CelebA_Baseline_trials.csv"))
