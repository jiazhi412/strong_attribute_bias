import torch
from models_eval.Custom_downstream_baseline import Model as Custom
...
class Model(Custom):
    ...
    def _train(self, loader):
        ...
        for i, (x, y, a) in enumerate(loader):
            ...
            if self.opt["filter_mode"] == "neutral":
    ...
    def test(self):
        ...
        if self.opt["test"] or self.epoch == self.opt["epochs"]:
            ...
            data = {"Time": [datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")], "Attr": [self.opt["attrs_pred"][0]], "Eval Mode": [self.opt["Custom_test_mode"]], "H(Y|A)": [self.ce], "p_bc": [self.opt["p_bc"]], "Test Acc": [test_acc * 100]}
            utils.append_data_to_csv(data, os.path.join("result", self.opt["experiment"], self.opt["name"], f"Custom_Ours ({ft})_trials.csv"))
