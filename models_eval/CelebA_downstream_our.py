import torch
from models_eval.CelebA_downstream_baseline import Model as CelebA
...
class Model(CelebA):
    ...
    def _train(self, loader):
        ...
        if self.epoch % 10 == 0:
            ...
            if self.opt["filter_mode"] == "reverse":
                utils.log_image("test/reverse images", reverse_images)
            utils.log_image("train/filtered images", x_filtered)
    ...
    def train(self):
        ...
        # evaluation
        train_loss, train_output, train_feature, train_target, train_a = self._test(self.train_loader)
        train_acc = utils.compute_Acc_withlogits_binary(train_output, train_target)
        dev_loss, dev_output, dev_feature, dev_target, dev_a = self._test(self.dev_loader)
    ...
    def test(self):
        ...
        test_loss, test_output, test_feature, test_target, test_a = self._test(self.test_loader)
        ...
        if self.opt["test"] or self.epoch == self.opt["epochs"]:
            ...
            data = {"Time": [datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")], "Attr": [self.opt["attrs_pred"][0]], "Eval Mode": [self.opt["CelebA_test_mode"]], "H(Y|A)": [self.ce], "p_bc": [self.opt["p_bc"]], "Test Acc": [test_acc * 100]}
            utils.append_data_to_csv(data, os.path.join("result", self.opt["experiment"], self.opt["name"], f"CelebA_Ours ({ft})_trials.csv"))
