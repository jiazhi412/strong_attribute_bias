...
from dataloader.CelebA_train import CelebA
from dataloader.FFHQ import FFHQ
from dataloader.CelebA_train_synthetic import CelebA_synthetic
...
from dataloader.CelebA_train import check_attribute_conflict
...
class Model:
    def __init__(self, args):
        ...
        self.mine = M(input_dim=(args.enc_dim * 2 ** (args.enc_layers - 1) + self.dim_attrs) * 7 * 7, hidden_dim=hidden_dim)
        self.mine.train()
        if self.gpu:
            self.mine.cuda()
        summary(self.mine, [(1, (args.enc_dim * 2 ** (args.enc_layers - 1) + self.dim_attrs) * 7 * 7)], batch_size=4, device="cuda" if args.gpu else "cpu")
    ...
    def set_data(self, args):
        if args.CelebA_train_mode == "CelebA_train":
            train_dataset = CelebA(args.data_path, args.attr_path, args.img_size, "train", args.CelebA_attrs)
            valid_dataset = CelebA(args.data_path, args.attr_path, args.img_size, "valid", args.CelebA_attrs)
        elif args.CelebA_train_mode == "CelebA_all":
            train_dataset = CelebA(args.data_path, args.attr_path, args.img_size, "all", args.CelebA_attrs)
            valid_dataset = CelebA(args.data_path, args.attr_path, args.img_size, "valid", args.CelebA_attrs)
        elif args.CelebA_train_mode == "CelebA_train_ex":
            train_dataset = CelebA(args.data_path, args.attr_path, args.img_size, "ex", args.CelebA_attrs, args.CelebA_attrs_d)
            valid_dataset = CelebA(args.data_path, args.attr_path, args.img_size, "valid", args.CelebA_attrs)
        elif args.CelebA_train_mode == "FFHQ":
            train_dataset = FFHQ(args.ffhq_path)
            valid_dataset = FFHQ(args.ffhq_path)
        elif args.CelebA_train_mode == "CelebA_FFHQ":
            CelebA_train_dataset = CelebA(args.data_path, args.attr_path, args.img_size, "all", args.CelebA_attrs)
            valid_dataset = CelebA(args.data_path, args.attr_path, args.img_size, "valid", args.CelebA_attrs)
            FFHQ_train_dataset = FFHQ(args.ffhq_path)
            train_dataset = torch.utils.data.ConcatDataset([CelebA_train_dataset, FFHQ_train_dataset])
        elif args.CelebA_train_mode == "CelebA_synthetic":
            train_dataset = CelebA_synthetic(args.data_path, args.attr_path, args.syn_data_path, args.img_size, "all", args.CelebA_attrs, args.CelebA_attrs_d)
            valid_dataset = CelebA_synthetic(args.data_path, args.attr_path, args.syn_data_path, args.img_size, "valid", args.CelebA_attrs, args.CelebA_attrs_d)
    ...
    def train_epoch(self, args):
        ...
        errG, errD = None, None
        for img_a, att_a in progressbar(self.train_dataloader):
            # prepare data
    ...
    def trainG(self, img_a, att_a, att_a_, att_b, att_b_):
        ...
        att_c = torch.ones_like(att_b) / 2  # att_a and att_b range from (0, 1)
        att_c_ = torch.zeros_like(att_b_)  # att_a_ and att_b_ range from (-1, 1)
    ...
    def trainD(self, img_a, att_a, att_a_, att_b, att_b_):
        ...
        # 2. classify the sex out from real image, train classifier
    ...
    def eval_model(self, valid_dataloader, args):
        ...
        for i in range(args.n_attrs):
            ...
            tmp[:, i] = 1 - tmp[:, i]
            tmp = check_attribute_conflict(tmp, args.CelebA_attrs[i], args.CelebA_attrs)
            sample_att_b_list.append(tmp)

        self.eval()
        with torch.no_grad():
            samples = [fixed_img_a]  # No.1 original image
            for i, att_b in enumerate(sample_att_b_list):
                att_b_ = (att_b * 2 - 1) * args.thres_int  # -1/2, 1/2 for all
                if i == 0:  # i == 0 is for reconstruction No.2 reconstruction image
                    samples.append(self.G(fixed_img_a, att_b_)) 
            samples.append(self.G(fixed_img_a, torch.zeros_like(att_b_)))  # No.3 filtered image
