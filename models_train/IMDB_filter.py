...
class Model:
    ...
    def set_data(self, args):
        data_folder = {
            "origin_image_feature_path": os.path.join(args.data_path, "IMDB.h5py"),
            "origin_target_dict_path": os.path.join(args.data_path, "age_dict"),
            "origin_sex_dict_path": os.path.join(args.data_path, "sex_dict"),
            "origin_eb1_key_list_path": os.path.join(args.data_path, "eb1_img_list"),
            "origin_eb2_key_list_path": os.path.join(args.data_path, "eb2_img_list"),
            "origin_unbiased_key_list_path": os.path.join(args.data_path, "test_img_list"),
        }
    ...
    def pretrainMI(self, mine_loader, args):
        ...
        for img_a, _, att_a in progressbar(mine_loader):
            ...
            a_tile = att_a.view(att_a.size(0), -1, 1, 1).repeat(1, self.dim_per_attr, self.f_size, self.f_size)
    ...
    def _criterion_regr(self, output, target):
        ...
        return L(output, target.type(torch.long).squeeze())
